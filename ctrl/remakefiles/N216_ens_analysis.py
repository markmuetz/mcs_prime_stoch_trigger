...
def regrid_imerg_to_n216_inputs(case, regrid_method):
...
def regrid_imerg_to_n216_outputs(case, regrid_method):
    times_args = conf.UM_TIMES[case]
    args, kwargs = times_args
    times = pd.date_range(*args, **kwargs)
...
@rule(
    ...
    inputs=regrid_imerg_to_n216_inputs,
    outputs=regrid_imerg_to_n216_outputs,
)
def regrid_imerg_to_n216(inputs, outputs, case, regrid_method):
...
def regrid_era5_to_n216_inputs(case, var):
    # Just load IMERG on the hour "S??0000".
    times_args = conf.UM_TIMES[case]
    args, kwargs = times_args
    times = pd.date_range(*args, **kwargs)
...
def regrid_era5_to_n216_outputs(case, var):
    times_args = conf.UM_TIMES[case]
    args, kwargs = times_args
    times = pd.date_range(*args, **kwargs)
    d0 = str(times[0]).replace(' ', '_')
...
@rule(
    ...
    inputs=regrid_era5_to_n216_inputs,
    outputs=regrid_era5_to_n216_outputs,
)
def regrid_era5_to_n216(inputs, outputs, case, var):
...
def plot_precip_snapshots_inputs(case, regrid_method, ens, red_cf_expt):
    inputs = {}
    ...
    inputs['imerg'] = regrid_imerg_to_n216_outputs(case, regrid_method)['output']
    return inputs


def plot_precip_snapshots_outputs(case, regrid_method, ens, red_cf_expt):
    if red_cf_expt:
        return {
...
@rule(
    ...
    inputs=plot_precip_snapshots_inputs,
    outputs=plot_precip_snapshots_outputs,
    depends_on=[regrid_imerg_to_n216],
)
def plot_precip_snapshots(inputs, outputs, case, regrid_method, ens, red_cf_expt):
    """Plot precipitation snapshots for the different expts."""
    figdir = Path(outputs['dummy']).parent
    ...
    Path(outputs['dummy']).write_text('done')


def calc_total_precip_inputs(case, regrid_method, ens, red_cf_expt):
    inputs = {}
    for expt in conf.EXPT_SIM:
        suite = conf.EXPT_SIM[expt]
        inputs[f'pflux_{expt}'] = (
...
def calc_total_precip_outputs(case, regrid_method, ens, red_cf_expt):
    if red_cf_expt:
        return {
...
@rule(
    ...
    inputs=calc_total_precip_inputs,
    outputs=calc_total_precip_outputs,
    depends_on=[regrid_imerg_to_n216],
)
def calc_total_precip(inputs, outputs, case, regrid_method, ens, red_cf_expt):
...
def plot_total_precip_outputs(case, regrid_method, ens, red_cf_expt):
    if red_cf_expt:
...
@rule(
    ...
    inputs=calc_total_precip_outputs,
    outputs=plot_total_precip_outputs,
    depends_on=[calc_total_precip],
)
def plot_total_precip(inputs, outputs, case, regrid_method, ens, red_cf_expt):
...
def guassian_filter_n216_imerg_inputs(case, regrid_method):
    inputs = {'imerg': regrid_imerg_to_n216_outputs(case, regrid_method)['output']}
    return inputs


def guassian_filter_n216_imerg_outputs(case, regrid_method):
...
@rule(
    ...
    inputs=guassian_filter_n216_imerg_inputs,
    outputs=guassian_filter_n216_imerg_outputs,
    depends_on=[regrid_imerg_to_n216],
    uses={'settings': settings},
)
def guassian_filter_n216_imerg(inputs, outputs, case, regrid_method):
...
def guassian_filter_expt_inputs(expt, case):
    suite = conf.EXPT_SIM[expt]
    inputs = {
        'pflux': (
            conf.SIMDIR / f'{suite}/share/cycle/{case}/engl/um/englaa_pa.merged.{case}.{suite}.m01s05i216.nc'
...
@rule(
    ...
    inputs=guassian_filter_expt_inputs,
    outputs=guassian_filter_expt_outputs,
    uses={'settings': settings},
)
def guassian_filter_expt(inputs, outputs, expt, case):
...
def calc_eRMSE_inputs(expt, case, regrid_method):
    inputs = {
        'pflux_filtered': guassian_filter_expt_outputs(expt, case)['pflux_filtered'],
        'imerg_filtered': guassian_filter_n216_imerg_outputs(case, regrid_method)['imerg_filtered'],
    }
    return inputs
...
@rule(
    ...
    inputs=calc_eRMSE_inputs,
    outputs=calc_eRMSE_outputs,
    depends_on=[guassian_filter_expt, guassian_filter_n216_imerg],
    uses={'settings': settings, 'rmse': rmse},
)
def calc_eRMSE(inputs, outputs, expt, case, regrid_method):
...
def calc_dRMSE_inputs(expt, case):
    inputs = {
        'pflux_filtered': guassian_filter_expt_outputs(expt, case)['pflux_filtered'],
    }
    return inputs
...
@rule(
    ...
    inputs=calc_dRMSE_inputs,
    outputs=calc_dRMSE_outputs,
    depends_on=[guassian_filter_expt],
    uses={'settings': settings, 'rmse': rmse},
)
def calc_dRMSE(inputs, outputs, expt, case):
...
def plot_spread_error_inputs(plot_kwargs, case, regrid_method):
    inputs = {
        f'{expt}_eRMSE': calc_eRMSE_outputs(expt, case, regrid_method)['eRMSE'] for expt in conf.EXPT_SIM
    }
    inputs.update({f'{expt}_dRMSE': calc_dRMSE_outputs(expt, case)['dRMSE'] for expt in conf.EXPT_SIM})
    return inputs
...
def plot_all_cases_spread_error_inputs(plot_kwargs, regrid_method):
    inputs = {
        f'{case}_{expt}_eRMSE': calc_eRMSE_outputs(expt, case, regrid_method)['eRMSE']
        for expt in conf.EXPT_SIM
        for case in conf.CASES
    }
    inputs.update(
        {
            f'{case}_{expt}_dRMSE': calc_dRMSE_outputs(expt, case)['dRMSE']
            for expt in conf.EXPT_SIM
            for case in conf.CASES
        }
    )
...
@rule(
    ...
    outputs=plot_all_cases_spread_error_outputs,
    depends_on=[calc_eRMSE, calc_dRMSE],
    uses={
...
def calc_autocorr_imerg_inputs(case):
    inputs = {'imerg': regrid_imerg_to_n216_outputs(case, 'cons')['output']}
    return inputs


def calc_autocorr_imerg_outputs(case):
    args, kwargs = conf.UM_TIMES[case]
    times = pd.date_range(*args, **kwargs)
    d0 = str(times[0]).replace(' ', '_')
    dlast = str(times[-1]).replace(' ', '_')
...
@rule(
    ...
    inputs=calc_autocorr_imerg_inputs,
    outputs=calc_autocorr_imerg_outputs,
    depends_on=[regrid_imerg_to_n216],
)
def calc_autocorr_imerg(inputs, outputs, case):
...
def calc_autocorr_expt_inputs(expt, case):
    suite = conf.EXPT_SIM[expt]
    inputs = {
        'pflux': (
            conf.SIMDIR / f'{suite}/share/cycle/{case}/engl/um/englaa_pa.merged.{case}.{suite}.m01s05i216.nc'
...
@rule(
    ...
    inputs=calc_autocorr_expt_inputs,
    outputs=calc_autocorr_expt_outputs,
)
def calc_autocorr_expt(inputs, outputs, expt, case):
...
def plot_autocorr_inputs(case):
    inputs = {}
    inputs['imerg'] = calc_autocorr_imerg_outputs(case)['imerg_autocorr']
    for expt in conf.EXPT_SIM:
        inputs[expt] = calc_autocorr_expt_outputs(expt, case)['pflux_autocorr']
    return inputs


def plot_autocorr_outputs(case):
    return {'fig': conf.PATHS['figdir'] / 'ensemble' / case / f'precip_autocorr.{case}.cons.pdf'}


@rule(
    matrix={'case': conf.CASES},
    inputs=plot_autocorr_inputs,
    outputs=plot_autocorr_outputs,
    depends_on=[calc_autocorr_imerg, calc_autocorr_expt],
)
def plot_autocorr(inputs, outputs, case):
...
def calc_tcwv_inputs(case):
    ...
    inputs['tcwv_era5'] = regrid_era5_to_n216_outputs(case, 'tcwv')['output']
    return inputs


def calc_tcwv_outputs(case):
    return {'tcwv_output': conf.PATHS['figdir'] / 'ensemble' / case / f'tcwv.{case}.cons.nc'}


@rule(
    matrix={'case': conf.CASES},
    inputs=calc_tcwv_inputs,
    outputs=calc_tcwv_outputs,
    depends_on=[regrid_era5_to_n216],
)
def calc_tcwv(inputs, outputs, case):
...
def plot_tcwv_outputs(case):
    return {'fig': conf.PATHS['figdir'] / 'ensemble' / case / f'tcwv.{case}.cons.pdf'}


@rule(
    matrix={'case': conf.CASES},
    inputs=calc_tcwv_outputs,
    outputs=plot_tcwv_outputs,
    depends_on=[calc_tcwv],
)
def plot_tcwv(inputs, outputs, case):
...
def calc_mcsp_calling_freq_data_inputs(case):
    inputs = {}
...
def calc_mcsp_calling_freq_data_outputs(case):
    return {'mcsp_calling_freq': conf.PATHS['outdir'] / 'ensemble' / case / f'MCSP_calling_freq.{case}.nc'}


@rule(
    matrix={'case': conf.CASES},
    inputs=calc_mcsp_calling_freq_data_inputs,
    outputs=calc_mcsp_calling_freq_data_outputs,
)
def calc_mcsp_calling_freq_data(inputs, outputs, case):
...
def plot_mcsp_calling_freq_outputs(case):
    return {'fig': conf.PATHS['figdir'] / 'ensemble' / case / f'MCSP_calling_freq.{case}.pdf'}


@rule(
    matrix={'case': conf.CASES},
    inputs=calc_mcsp_calling_freq_data_outputs,
    outputs=plot_mcsp_calling_freq_outputs,
    depends_on=[calc_mcsp_calling_freq_data],
)
def plot_mcsp_calling_freq(inputs, outputs, case):
...
def first_look_plot_era5_500hpa_geopotential_inputs(case, time_offset):
    month = case[4:6]
    inputs = {
        'era5geopot': (
            conf.PATHS['datadir']
...
def first_look_plot_era5_500hpa_geopotential_outputs(case, time_offset):
    times_args = conf.UM_TIMES[case]
    args, kwargs = times_args
    times = pd.date_range(*args, **kwargs)
    d0 = str(times[0]).replace(' ', '_')
...
@rule(
    ...
    inputs=first_look_plot_era5_500hpa_geopotential_inputs,
    outputs=first_look_plot_era5_500hpa_geopotential_outputs,
    uses={'settings': settings},
)
def first_look_plot_era5_500hpa_geopotential(inputs, outputs, case, time_offset):
    ...
    for expt, sim in conf.EXPT_SIM.items():
        ...
        for i in range(10):
            ...
            try:
                ...
                Path(outputs[f'{expt}fig_{i}']).touch()
            plt.close('all')


def calc_geopot_dRMSE_inputs(expt, case, domain):
    suite = conf.EXPT_SIM[expt]
    inputs = {
        'geopot': (
...
@rule(
    ...
    inputs=calc_geopot_dRMSE_inputs,
    outputs=calc_geopot_dRMSE_outputs,
    uses={'settings': settings, 'rmse': rmse},
)
def calc_geopot_dRMSE(inputs, outputs, expt, case, domain):
...
def calc_geopot_eRMSE_inputs(expt, case, domain):
    suite = conf.EXPT_SIM[expt]
    month = case[4:6]
    if domain == 'global':
        inputs = {
            'era5geopot': regrid_era5_to_n216_outputs(case, 'z')['output'],
            'simgeopot': (
                conf.SIMDIR
...
@rule(
    ...
    inputs=calc_geopot_eRMSE_inputs,
    outputs=calc_geopot_eRMSE_outputs,
    depends_on=[regrid_era5_to_n216],
    uses={'settings': settings, 'rmse': rmse},
)
def calc_geopot_eRMSE(inputs, outputs, expt, case, domain):
...
def plot_geopot_spread_error_inputs(plot_kwargs, case, domain):
    inputs = {
        f'{expt}_eRMSE': calc_geopot_eRMSE_outputs(expt, case, domain)['eRMSE'] for expt in conf.EXPT_SIM
    }
    inputs.update(
        {f'{expt}_dRMSE': calc_geopot_dRMSE_outputs(expt, case, domain)['dRMSE'] for expt in conf.EXPT_SIM}
    )
    return inputs
...
def plot_all_cases_geopot_spread_error_inputs(plot_kwargs, domain):
    inputs = {
        f'{case}_{expt}_eRMSE': calc_geopot_eRMSE_outputs(expt, case, domain)['eRMSE']
        for expt in conf.EXPT_SIM
        for case in conf.CASES
    }
    inputs.update(
        {
            f'{case}_{expt}_dRMSE': calc_geopot_dRMSE_outputs(expt, case, domain)['dRMSE']
            for expt in conf.EXPT_SIM
            for case in conf.CASES
        }
    )
...
rmk.add_rules([
    regrid_imerg_to_n216,
    regrid_era5_to_n216,
    plot_precip_snapshots,
    calc_total_precip,
    plot_total_precip,
    guassian_filter_n216_imerg,
    guassian_filter_expt,
    calc_eRMSE,
    calc_dRMSE,
    plot_spread_error,
    plot_all_cases_spread_error,
    # calc_autocorr_imerg, calc_autocorr_expt, plot_autocorr intentionally
    # excluded: enabled = False in the remake2 original.
    calc_tcwv,
    # plot_tcwv intentionally excluded: enabled = False in the remake2 original.
    calc_mcsp_calling_freq_data,
    plot_mcsp_calling_freq,
    first_look_plot_era5_500hpa_geopotential,
    calc_geopot_dRMSE,
    calc_geopot_eRMSE,
    plot_geopot_spread_error,
    plot_all_cases_geopot_spread_error,
])
